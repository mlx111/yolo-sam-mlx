> 状态说明：本文是早期开发基线和旧 experiment-sim-wrapper/UR5e 线迁移规划，
> 不是当前 R1Pro `experience_system` 的实现证据。
> 当前可复现主线以 `experience_system/docs/paper_experience_system.md`、
> `experience_system/docs/sandbox_fidelity_optimization_roadmap.md` 和
> `results/memory/universal_pipeline_calibration_v1/paper_evidence_summary.json` 为准。
> 文中真机闭环和真实成功率相关内容应视为未来工作。
