...
def default_r1pro_skill_semantics() -> dict[str, SkillSemantics]:
    ...
    entries = [
        ...
        _semantics(
            ...
            description="Set the torso height level to improve single-arm reachability.",
        ),
        _semantics(
        ...
        _semantics(
            ...
            description="Move the mobile base with either world-frame absolute targets or base-frame relative displacement.",
        ),
        _semantics(
...
def default_galaxea_field_atomic_skill_semantics() -> dict[str, SkillSemantics]:
    """Return precondition/effect semantics for Galaxea field-atomic skills."""

    entries = [
        _semantics(
            "move_base_relative",
            effects=["base_pose_changed"],
            description="Move the mobile base by a relative x/y command.",
        ),
        _semantics(
            "set_torso_posture",
            effects=["torso_posture_changed"],
            description="Set torso height to a fixed posture level.",
        ),
        _semantics(
            "head_camera_rgbd_save",
            effects=["rgbd_observation_available"],
            description="Capture RGB-D from the head camera.",
        ),
        _semantics(
            "head_camera_grounded_sam2_pose",
            requires=["rgbd_observation_available"],
            effects=["target_pose_known"],
            description="Estimate and save the target_class pose.",
        ),
        _semantics(
            "plan_cartesian_trajectory",
            requires=["target_pose_known"],
            effects=["pregrasp_trajectory_planned"],
            description="Plan a Cartesian trajectory to pregrasp.",
        ),
        _semantics(
            "move_to_pregrasp",
            requires=["target_pose_known"],
            effects=["pregrasp_reached"],
            optional_requires=["pregrasp_trajectory_planned"],
            description="Move the selected arm to pregrasp.",
        ),
        _semantics(
            "approach_object",
            requires=["target_pose_known", "pregrasp_reached"],
            effects=["grasp_pose_reached"],
            description="Approach from pregrasp to the final grasp point.",
        ),
        _semantics(
            "close_gripper",
            requires=["grasp_pose_reached"],
            effects=["gripper_closed", "object_grasp_attempted"],
            description="Close the selected gripper.",
        ),
        _semantics(
            "lift",
            requires=["gripper_closed"],
            effects=["lift_attempted"],
            optional_requires=["object_grasp_attempted"],
            description="Lift after gripper closure.",
        ),
        _semantics(
            "transport_to_detected_target",
            requires=["lift_attempted", "target_pose_known"],
            effects=["transport_attempted"],
            description="Move toward a previously detected target position.",
        ),
        _semantics(
            "lower_held_object",
            requires=["transport_attempted"],
            effects=["place_lowered"],
            optional_requires=["gripper_closed", "lift_attempted"],
            description="Lower the held object before release.",
        ),
        _semantics(
            "open_gripper",
            effects=["gripper_opened", "object_released"],
            optional_requires=["gripper_closed", "lift_attempted", "transport_attempted", "place_lowered"],
            consumes=["gripper_closed"],
