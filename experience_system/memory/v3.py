...
from .scoring import entry_risk_adjustment
...
class MemoryV3Entry:
    ...
    def __post_init__(self) -> None:
        ...
        if self.result.task_success is False:
            self.result.success = False
        if self.result.success is None:
            self.result.success = bool(self.result.task_success)
        self._coerce_episode_fields()
        self.status = "success" if self.result.success else "failure"
        self.domain = self.domain or self.source or "simulation"
        self._promote_metadata_defaults()
        if self.result.success is False and self.validation_status in {"simulation_only", "simulation_validated", "real_executed", "real_validated"}:
        ...
        if self.result.success is False and self.memory_partition in {"simulation_memory", "validated_memory", "real_memory"}:
            self.memory_partition = "failed_memory"
    ...
    def _promote_metadata_defaults(self) -> None:
        ...
        if not _dataclass_has_values(self.execution_feedback):
            self.execution_feedback = ExecutionFeedback(
                task_success=self.result.task_success if self.result.task_success is not None else None,
...
class MemoryV3Library:
        ...
        **_: Any,
        ...
        for entry in self._entries:
            if entry.scenario_id != scenario_id:
                continue
            action_steps = candidate_failure_steps(entry) or entry.skill_sequence
            actions = {str(step.get("action", "")) for step in action_steps if isinstance(step, dict)}
            if not actions.issubset(allowed):
                continue
            if not include_failed and not entry.result.success:
                continue
    ...
    def _score(
        ...
        *,
        ...
        result_success = float(entry.result.success)
    ...
    def _should_promote(self, entry: MemoryV3Entry) -> bool:
        ...
        if entry.result.success and retrieval_count >= self.MIN_RETRIEVAL_FOR_PROMOTION:
            return True
        return False
    ...
    def consolidate(self) -> dict[str, Any]:
        ...
        if over > 0:
            def _eviction_key(e: MemoryV3Entry) -> tuple:
                protection = 0 if e.result.success else 1
...
def _json_safe(value: Any) -> Any:
    """Convert nested numpy values to plain JSON-compatible objects."""
...
def infer_memory_partition(entry: MemoryV3Entry) -> str:
    ...
    if validation_status in {"failed", "failure"} or not entry.result.success:
...
def build_retrieval_key(entry: MemoryV3Entry) -> dict[str, Any]:
    ...
    criteria = feedback.get("task_success_criteria") or {}
    ...
    return {
        ...
        "task_success": bool(criteria.get("success", entry.result.task_success)),
...
def build_text_summary(entry: MemoryV3Entry) -> str:
    ...
    if ef and hasattr(ef, "__dataclass_fields__"):
        criteria = getattr(ef, "task_success_criteria", None) or {}
    else:
        criteria = {}
...
def default_confidence_score(entry: MemoryV3Entry) -> float:
    if entry.validation_status in {"real_validated", "simulation_validated"} and entry.result.success:
        return 0.85
    if entry.result.success:
        return 0.7
    return 0.35
...
def _parse_action_sequence(entry: MemoryV3Entry | Any) -> list[str]:
    ...
    Each element is like "close_gripper" or "approach_object",
...
def canonical_action_signature_from_entry(entry: Any) -> str:
    candidate_steps = candidate_failure_steps(entry)
    if candidate_steps:
        return canonical_action_signature_from_steps(candidate_steps)
    return canonical_action_signature_from_steps(getattr(entry, "skill_sequence", []) or [])


def candidate_failure_steps(entry: Any) -> list[dict[str, Any]]:
    ...
    rejections = candidate_failure.get("rejections") if isinstance(candidate_failure, dict) else []
    ...
    for item in metadata.get("candidate_rejections") or []:
        if not isinstance(item, dict):
            continue
        steps = item.get("steps") if isinstance(item.get("steps"), list) else []
        usable = [
    ...
    *,
    ...
    return MemoryV3Entry(
        ...
        result=MemoryV3Result(
            ...
            success=(bool(task_success) if task_success is not None else None) if success is None else bool(success),
        ...
        validation_status=validation_status or ("simulation_only" if task_success else "failed"),
