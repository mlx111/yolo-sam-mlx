...
def format_elapsed_seconds(elapsed_seconds: float | None) -> str:
...
def print_action_result(step_no: int, total_steps: int, mode: str, action: str, response: Dict[str, Any]) -> None:
    ...
    if "elapsed_seconds" in response:
        print(f"  elapsed={format_elapsed_seconds(response['elapsed_seconds'])}")
    ...
    if isinstance(payload, dict):
        ...
        if "rule_status" in payload:
            print(f"  rule_status={payload.get('rule_status')}")
        if "recheck_used" in payload:
            print(f"  recheck_used={payload.get('recheck_used')}")
...
def run_task_items(task_items: List[Dict[str, Any]], *, mode: str, source_name: str, base_url: str) -> Dict[str, Any]:
    ...
    for step_no, item in enumerate(task_items, start=1):
        ...
        response["elapsed_seconds"] = time.perf_counter() - step_started_at
...
def score(*, base_url: str) -> Dict[str, Any]:
    ...
    response["elapsed_seconds"] = time.perf_counter() - started_at
...
def run_pipeline(task_file: str, recovery_file: str, *, base_url: str) -> int:
    ...
    setup_result["elapsed_seconds"] = time.perf_counter() - setup_started_at
    ...
    if should_stop_for_failure(setup_result):
        ...
        print(f"setup_elapsed={format_elapsed_seconds(setup_result.get('elapsed_seconds'))}")
        print(f"setup_message={setup_result.get('message')}")
        print("main_success=False")
        print("recovery_triggered=False")
        print("main_elapsed=skipped")
        print("recovery_elapsed=skipped")
        print("score_elapsed=skipped")
        print("score_status=skipped")
        print("score_message=reset-environment failed")
        print(f"total_elapsed={format_elapsed_seconds(time.perf_counter() - pipeline_started_at)}")
        return 1
    ...
    print(f"main_elapsed={format_elapsed_seconds(main_result.get('elapsed_seconds'))}")
    ...
    if recovery_result is not None:
        ...
        print(f"recovery_elapsed={format_elapsed_seconds(recovery_result.get('elapsed_seconds'))}")
        ...
        print("recovery_elapsed=skipped")
    print(f"score_elapsed={format_elapsed_seconds(score_result.get('elapsed_seconds'))}")
