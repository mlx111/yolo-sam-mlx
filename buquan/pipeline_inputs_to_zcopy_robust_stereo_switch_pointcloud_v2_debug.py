...
4) Save raw/centered/aligned debug point clouds.
5) Strictly copy points along Z (no interpolation).
"""
...
def run(cfg: Config) -> Dict[str, object]:
    ...
    out_raw_points = os.path.join(cfg.output_dir, f"{cfg.object_name}_raw_points.ply")
    save_points(out_raw_points, raw_points)
    ...
    out_centered_raw = os.path.join(cfg.output_dir, f"{cfg.object_name}_centered_raw.ply")
    out_aligned_raw = os.path.join(cfg.output_dir, f"{cfg.object_name}_aligned_raw.ply")
    save_points(out_centered_raw, raw_centered)
    save_points(out_aligned_raw, raw_aligned)
    ...
    report = {
        ...
        "outputs": {
            "raw_points_ply": out_raw_points,
            "centered_raw_ply": out_centered_raw,
            "aligned_raw_ply": out_aligned_raw,
...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(description="Inputs image -> pointcloud_v2 raw-point export -> preprocess -> align -> strict Z-copy fill with raw/centered/aligned debug exports.")
