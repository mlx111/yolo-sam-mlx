...
"""
One-shot runner:
1) image -> robust z-copy filled point cloud (pear can be canonicalized to +Y tip)
...
PIPELINE_SCRIPT = os.path.join(SCRIPT_DIR, "pipeline_inputs_to_zcopy_robust_stereo_switch.py")
...
def main() -> int:
    ...
    cmd_stage1 = [
        ...
        "--pear-y-canonical",
        str(args.pear_y_canonical),
        "--pear-tip-percentile",
        f"{float(args.pear_tip_percentile)}",
        "--output-dir",
        zcopy_out_dir,
    ]
