...
sys.path.insert(0, str(ROOT.parent / 'manipulator_grasp'))
...
from memory.calibration import apply_calibration_to_position, calibrated_attach_distance
...
class SimWrapper:
    ...
    def plan_recovery_in_virtual(self, virtual_scene, recovery_pos) -> tuple[bool, list[dict]]:
        ...
        if not self._v_attach_body(scene, "apple0", max_distance=getattr(scene, "attach_max_distance", GRASP_ATTACH_MAX_DISTANCE)):
            scene.virtual_failure = {
                "reason": "attach_gate_failed",
                "attach_attempt": scene.last_attach_attempt,
            }
            return False, []
    ...
    def _v_move_joints(scene, q_target, duration=1.0):
        ...
        for t in np.linspace(0, duration, int(duration / SIM_TIMESTEP)):
            ...
            SimWrapper._v_track_body(scene)
            if scene.viewer:
                scene.viewer.sync()
    ...
    def _v_attach_body(scene, body_name, max_distance=GRASP_ATTACH_MAX_DISTANCE):
        ...
        if pinch_distance > float(max_distance):
            ...
            return False
        scene.grasp_offset_pos = body_pos - pinch_pos
        scene.tracked_body_id = body_id
        scene.tracked_body_adr = adr
        scene.grasp_tracking = True
        scene.last_attach_attempt = {
            "body": body_name,
            "success": True,
            "pinch_distance": pinch_distance,
    ...
    @staticmethod
    def _v_track_body(scene):
        if not scene.grasp_tracking or scene.grasp_offset_pos is None or scene.tracked_body_adr is None:
            return
        pinch_pos = scene.data.site_xpos[scene.pinch_site_id].copy()
        scene.data.qpos[scene.tracked_body_adr:scene.tracked_body_adr + 3] = pinch_pos + scene.grasp_offset_pos
        mujoco.mj_forward(scene.model, scene.data)

    @staticmethod
    def _v_detach_body(scene):
        scene.grasp_tracking = False
        scene.tracked_body_id = None
        scene.tracked_body_adr = None
        scene.grasp_offset_pos = None
