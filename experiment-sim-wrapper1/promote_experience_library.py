...
def _normalize_signature(sig: Any) -> str:
    ...
    (``[{"action": "move-grasp", "state": 1}, ...]``) or as a JSON string.
...
def _trial_results(results_dir: Path) -> list[tuple[Path, dict[str, Any]]]:
    """Walk batch experiment result trees.

    New-format (primary):  scenario_id / condition_id / method / trial_XXX / result.json
    Old-format (fallback): anomaly / method / trial_XXX / result.json
    """
...
def _promotion_eligible(data: dict[str, Any]) -> bool:
    ...
    ``recovery_success`` and ``virtual_validation_success`` are required.
    ``task_success`` may be ``None`` (not evaluated) — only block promotion
    when it is explicitly ``False``.
    """
    ts = data.get("task_success")
    task_ok = ts is True if ts is not None else True
    return bool(
        data.get("virtual_validation_success") is True
        and data.get("recovery_success") is True
        and task_ok
...
def promote(
    ...
    for entry in lib:
        ...
        if partition == "failed_memory" or not entry.result.recovery_success:
            from_partition = partition
        ...
        elif partition == "simulation_memory" and entry.result.recovery_success:
