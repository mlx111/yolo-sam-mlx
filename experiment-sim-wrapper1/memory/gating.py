...
The gate is intentionally deterministic and side-effect free.  At this phase it
only records why an episode is valuable; callers still decide whether to save.
"""
...
def compute_memory_gate(
    ...
    *,
    ...
    Scores are normalized to [0, 1].  The returned dictionary matches
    memory.v3.MemoryGateInfo, so it can be serialized directly into entries.
    """
    ...
    if recovery_success is None:
        raw_recovery = metrics.get("recovery_success")
        recovery_success = bool(raw_recovery) if raw_recovery is not None else None
    ...
    if recovery_success is False:
        failure_score = 1.0
        _event(trigger_events, "recovery_failed")
    ...
    recovery_criteria = metrics.get("recovery_success_criteria") or {}
    task_criteria = metrics.get("task_success_criteria") or {}
    has_structured_evidence = bool(recovery_criteria or task_criteria or metrics.get("skill_results"))

    if recovery_success is True:
        _event(trigger_events, "recovery_success")
    ...
    if anomaly_detected and recovery_success is False:
    ...
    z_change = recovery_criteria.get("z_change")
    try:
