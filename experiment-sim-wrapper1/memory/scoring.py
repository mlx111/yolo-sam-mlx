...
def _result_success(entry: Any) -> bool:
    ...
    return bool(getattr(result, "success", False) or getattr(result, "recovery_success", False))
...
def _signature_actions_from_steps(steps: Any) -> list[str]:
    ...
    for step in steps or []:
        ...
        if action == "gripper-action":
            action = f"{action}:{params.get('state', '')}"
        if action:
...
def estimate_real_success_prior(entries: list[tuple[Any, float]]) -> dict[str, Any]:
    ...
    for entry, score in entries or []:
        ...
        total += weight
        result = getattr(entry, "result", None)
        ok = bool(getattr(result, "success", False) or getattr(result, "recovery_success", False))
...
def score_candidate_plan(
    ...
    for entry, score in retrieved_experiences or []:
        ...
        result = getattr(entry, "result", None)
        entry_success = bool(getattr(result, "success", False) or getattr(result, "recovery_success", False))
