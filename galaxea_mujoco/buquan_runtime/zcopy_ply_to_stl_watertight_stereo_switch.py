...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=["on", "off"],
        default="off",
        help="Force mesh to be built from convex hull directly.",
    )
...
def postprocess_mesh(
    ...
    if target_faces > 0 and len(out.faces) > int(target_faces):
        try:
            out = out.simplify_quadric_decimation(face_count=int(target_faces))
            out = clean_mesh(out)
        except Exception:  # noqa: BLE001
            pass
...
def main() -> int:
    ...
    force_convex_hull = str(args.force_convex_hull).strip().lower() == "on"
    if force_convex_hull:
        mesh = clean_mesh(trimesh.points.PointCloud(points).convex_hull)
        fallback_used = True
    else:
