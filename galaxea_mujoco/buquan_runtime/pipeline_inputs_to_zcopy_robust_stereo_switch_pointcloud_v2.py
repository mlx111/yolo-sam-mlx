...
POINTCLOUD_V2_INTRINSICS = {
    "left": (385.78, 385.22, 327.58, 238.69),
    "right": (385.78, 385.22, 327.58, 238.69),
}


def _derive_color_rel_from_depth_rel(depth_rel: str) -> str:
    base = os.path.basename(depth_rel)
    if base.startswith("d"):
        return os.path.join(os.path.dirname(depth_rel), "c" + base[1:])
    raise ValueError(f"Cannot derive color filename from depth path: {depth_rel}")


def build_raw_points_with_pointcloud_v2(
    camera: str,
    color_path: str,
    depth_path: str,
    mask_path: str,
    ...
    fx, fy, cx, cy = POINTCLOUD_V2_INTRINSICS[camera]
    euler_deg = CAMERA_EULER_DEG[camera]
    camera_to_tcp_pose = [
        0.0,
        0.0,
        0.0,
        float(euler_deg[0]) / 180.0 * np.pi,
        float(euler_deg[1]) / 180.0 * np.pi,
        float(euler_deg[2]) / 180.0 * np.pi,
    ]
    ...
    response = generator.generate_point_cloud(
        ...
        image_alignment_scale=1.0,
        objects=os.path.splitext(os.path.basename(mask_path))[0],
        flag=camera,
        type1='normal',
    )
    if response.get('state') != 'success' or response.get('point_cloud_cam') is None:
        raise ValueError(f"pointcloud_v2 failed to generate point_cloud_cam from {depth_path}: {response.get('info')}")
    intrinsics = np.array([[fx, 0.0, cx], [0.0, fy, cy], [0.0, 0.0, 1.0]], dtype=np.float64)
    return np.asarray(response['point_cloud_cam'], dtype=np.float64), intrinsics
...
def load_sim_intrinsics(path: str, camera: str) -> np.ndarray:
    if not path:
        raise ValueError("--sim-intrinsics-json is required when --camera-model sim")
    if not os.path.exists(path):
        raise FileNotFoundError(f"Sim intrinsics JSON not found: {path}")
    with open(path, "r", encoding="utf-8") as f:
        payload = json.load(f)
    block = payload.get(camera)
    if not isinstance(block, dict):
        raise ValueError(f"Sim intrinsics JSON missing camera block {camera!r}: {path}")
    try:
        fx = float(block["fx"])
        fy = float(block["fy"])
        cx = float(block["cx"])
        ...
        raise ValueError(f"Sim intrinsics JSON missing key {exc.args[0]!r}: {path}") from exc
...
def run(cfg: Config) -> Dict[str, object]:
    ...
    calib = None if cfg.camera_model == "sim" else load_calibration_from_pdf(cfg.calib_pdf)
    t_refined = None if cfg.camera_model == "sim" else load_refined_extrinsics(cfg.extrinsics_json)

    if cfg.camera == "right":
        depth_rel = cfg.right_depth_rel
        mask_tpl = cfg.right_mask_tpl
        selected_k = None if calib is None else calib["k_right"]
        selected_k_name = "k_right"
    else:
        depth_rel = cfg.left_depth_rel
        mask_tpl = cfg.left_mask_tpl
        selected_k = None if calib is None else calib["k_left"]
        selected_k_name = "k_left"

    depth_path = os.path.join(cfg.input_root, depth_rel)
    color_rel = _derive_color_rel_from_depth_rel(depth_rel)
    color_path = os.path.join(cfg.input_root, color_rel)
    ...
    if cfg.camera_model == "sim":
        selected_k = load_sim_intrinsics(cfg.sim_intrinsics_json, cfg.camera)
        ...
        selected_k_name = f"sim_{cfg.camera}_intrinsics"
        raw_points_source = "sim_metric_depth_mask_pinhole"
    else:
        raw_points, selected_k = build_raw_points_with_pointcloud_v2(
            camera=cfg.camera,
            color_path=color_path,
            depth_path=depth_path,
            mask_path=mask_path,
        )
        selected_k_name = f"pointcloud_v2_{cfg.camera}_intrinsics"
        raw_points_source = "pointcloud_v2.point_cloud_cam"
    ...
    calibration_report = {
        ...
        "raw_points_source": raw_points_source,
    }
    if cfg.camera_model == "sim":
        inputs_report["sim_intrinsics_json"] = cfg.sim_intrinsics_json
    else:
        inputs_report.update(
            {
...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(description="Inputs image -> pointcloud_v2 raw-point source -> preprocess -> align -> strict Z-copy fill.")
    ...
    parser.add_argument("--camera-model", choices=["pointcloud_v2", "sim"], default="pointcloud_v2")
    parser.add_argument("--sim-intrinsics-json", default="")
    ...
    return Config(
        ...
        sim_intrinsics_json=os.path.abspath(args.sim_intrinsics_json) if args.sim_intrinsics_json else "",
