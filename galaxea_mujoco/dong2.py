...
PEAR = "pear"
TABLE_TOP_Z = 0.75
ROBOT_ROOT_POS = [0.079032, 0.0, 0.0]
TABLE_CONTACT_CLEARANCE = 0.001
...
def _scene_template() -> Path:
    return ROOT_DIR / "scence" / "scene2.xml"


def _scene_output() -> Path:
    return ROOT_DIR / "scence" / "real_camera_runtime_scene.xml"
...
def _mesh_candidates(mesh_name: str) -> list[Path]:
    return [
        ROOT_DIR / "model" / "meshes" / "runtime_completed" / f"{mesh_name}_completed.stl",
        ROOT_DIR / "model" / "meshes" / "runtime_completed" / f"{mesh_name}_watertight_voxel_restored.stl",
        ROOT_DIR / "model" / "meshes" / "runtime_completed" / f"{mesh_name}_watertight_voxel.stl",
        _fruit_mesh_dir() / f"{mesh_name}.stl",
    ]


def _normalize_object_name(name: str) -> str:
    normalized = str(name).strip().lower().rstrip(".").replace(" ", "_")
    if normalized == "eyegalsses":
        return "eyeglasses"
    return normalized
...
def _tabletop_body_z(
    ...
    try:
        ...
        if quat is not None:
            rotation = _quat_to_matrix(quat, "tabletop_mesh")
            vertices = (rotation @ vertices.T).T
        z_min = float(vertices[:, 2].min())
        return TABLE_TOP_Z - z_min + TABLE_CONTACT_CLEARANCE
    except Exception:
        return max(float(fallback_z), TABLE_TOP_Z)
...
def _apply_robot_root_pose(spec: mujoco.MjSpec) -> None:
    for body in spec.worldbody.bodies:
        if body.name == "mobile_base_x":
            body.pos = ROBOT_ROOT_POS
            return
...
def _add_box(
    ...
    body_name = f"{safe_prefix}_{index}"
    body = spec.worldbody.add_body(
        name=body_name,
        quat=[1.0, 0.0, 0.0, 0.0],
        pos=pos,
...
def _ensure_mesh(spec: mujoco.MjSpec, object_name: str, mesh_names: set[str]) -> str:
    ...
    mesh_path = next((path for path in _mesh_candidates(mesh_name) if path.exists()), None)
    if mesh_path is None:
        searched = "\n".join(str(path) for path in _mesh_candidates(mesh_name))
        raise SceneGenerationError(f"Missing mesh for {object_name}. Searched:\n{searched}")
    spec.add_mesh(file=str(mesh_path), name=mesh_name)
    mesh_names.add(mesh_name)
    return mesh_name


def _resolve_mesh_path(object_name: str) -> Path | None:
    if object_name.endswith("box"):
        return None
    mesh_name = "eyeglasses" if object_name == "eyegalsses" else object_name
    return next((path for path in _mesh_candidates(mesh_name) if path.exists()), None)
...
def generate_scene(
    ...
    for object_name, raw_pos in result.items():
        ...
        if normalized_name.endswith("box"):
            ...
            pos[2] = max(pos[2], TABLE_TOP_Z + 0.035 + TABLE_CONTACT_CLEARANCE)
            _add_box(spec, pos, count, rgba, name_prefix=normalized_name)
        ...
        final_quat = _compose_quats(quat, mesh_quat, f"{normalized_name}{count}", [1.0, 0.0, 0.0, 0.0])
        pos[2] = _tabletop_body_z(_resolve_mesh_path(normalized_name), pos[2], quat=final_quat)

        if normalized_name in {APPLE, PEAR} and (quat is not None or mesh_quat is not None):
