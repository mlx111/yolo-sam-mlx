...
from .atomic_schema import FR5FieldAtomicResult
...
class FR5FieldAtomicSkillExecutor:
    def __init__(self, runtime: FR5SkillContext | Any, *, default_pregrasp_height: float = 0.08) -> None:
        self.context = runtime if isinstance(runtime, FR5SkillContext) else FR5SkillContext(runtime, default_pregrasp_height=default_pregrasp_height)
    ...
    def execute(self, action: str, parameters: dict[str, Any] | None = None) -> FR5FieldAtomicResult:
        action = str(action)
        params = parameters if isinstance(parameters, dict) else {}
        if action not in self._registry:
            return FR5FieldAtomicResult(action=action, success=False, status="unsupported_action", message=f"unsupported FR5 action: {action}", parameters=dict(params))
        skill_results_before = self._skill_result_count()
        try:
            raw = self._loaders[action]().execute_recovery_action(self.context, params)
        except NotImplementedError as exc:
            return FR5FieldAtomicResult(action=action, success=False, status="not_implemented", message=str(exc), parameters=dict(params), raw_result={})
        except Exception as exc:
            return FR5FieldAtomicResult(action=action, success=False, status="failed", message=str(exc), parameters=dict(params), raw_result={})
    ...
    def execute_plan(self, steps: list[dict[str, Any]]) -> list[FR5FieldAtomicResult]:
        results: list[FR5FieldAtomicResult] = []
        for step in steps:
            if not isinstance(step, dict):
                results.append(FR5FieldAtomicResult(action="", success=False, status="invalid_step", message="step is not a dict"))
                continue
    ...
    def _build_loaders() -> dict[str, Callable[[], Any]]:
        ...
        from skills.primitives.go_camera_ready_skill import load_skill as load_go_camera_ready
        ...
        return {
            ...
            "go_camera_ready": load_go_camera_ready,
