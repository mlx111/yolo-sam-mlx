"""FR5 field-atomic skill registry.

This mirrors the UR5e registry surface so shared experience-system code can
read the same action and parameter contracts for either robot.
"""
...
def skill_description(action: str) -> str:
    ...
    return spec.description if spec is not None else "FR5 MuJoCo recovery skill."
...
def build_action_map(experiment: Any, default_pregrasp_height: float = 0.08, scenario_id: str | None = "") -> dict[str, Callable[[dict], Any]]:
    context = FR5SkillContext(experiment, default_pregrasp_height=default_pregrasp_height)
    executor = FR5FieldAtomicSkillExecutor(context, default_pregrasp_height=default_pregrasp_height)
