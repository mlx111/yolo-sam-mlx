...
os.environ.setdefault("EXPERIENCE_RUNTIME_SKILLS_ROOT", str(WRAPPER_ROOT))
...
DEFAULT_SCENE = WRAPPER_ROOT / "assets" / "scene.xml"
...
def _stable_experience_id(*parts: Any) -> str:
    ...
    return "fr5_smoke_" + hashlib.sha1(text.encode("utf-8")).hexdigest()[:12]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run FR5 field-atomic skills from a JSON action file.")
    ...
    parser.add_argument("--save-report", type=Path, default=WRAPPER_ROOT / "results" / "memory" / "field_atomic_skill_report.json")
    parser.add_argument("--scenario-id", default="fr5_field_atomic_smoke")
    parser.add_argument("--condition-id", default="default")
    parser.add_argument("--viewer", action="store_true")
    parser.add_argument("--camera-ready", action=argparse.BooleanOptionalAction, default=True, help="Move FR5 to ee_camera observation posture before executing the action file.")
...
def _write_failed_experience(
    *,
    ...
    entry = make_memory_v3_entry(
        ...
        failure_reason=failure_reason,
        source="simulation",
        summary=f"Failed FR5 field atomic smoke sequence: {failure_reason}",
        metadata={
            "source_tool": "fr5_run_field_atomic_skill_smoke",
...
def main() -> None:
    ...
    runtime = FR5MotionRuntime.from_scene(str(args.scene_xml), realtime=bool(args.viewer))
    runtime.reset_home()
    runtime.metrics["scenario_id"] = args.scenario_id
    runtime.metrics["condition_id"] = args.condition_id
    executor = FR5FieldAtomicSkillExecutor(runtime, default_pregrasp_height=DEFAULT_PREGRASP_HEIGHT)
    viewer = None
    if args.viewer:
    ...
    if args.camera_ready:
        init_result = executor.execute("go_camera_ready", {})
        if not init_result.success:
            raise RuntimeError(f"failed to initialize FR5 camera-ready posture: {init_result.message}")
        runtime.metrics["skill_results"] = []
    reports: list[dict[str, Any]] = []
    ...
    try:
        for index, step in enumerate(steps):
            ...
            if args.stop_on_failure and not bool(result.success):
                ...
                break
        report_payload = {
            "schema_version": "fr5_field_atomic_skill_smoke_report_v1",
            ...
            "actions": reports,
            "skill_results": runtime.metrics.get("skill_results", []),
